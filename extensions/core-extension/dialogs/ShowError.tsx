...
export function ShowError({
  ...
  switch (errorType) {
    ...
    case ERROR_PICKING_DIRECTORY_ERROR: {
      title = 'Произошла ошибка при открытии папки с заметками.';
      content = (
        <div>
          Убедитесь, что папка с заметками находится в общедоступном месте, например, «Документы» или «Рабочий стол».
        </div>
      );
      break;
    }
    ...
    case WORKSPACE_AUTH_REJECTED_ERROR: {
      title = 'Flent был лишен доступа к вашим заметкам.';
      content = (
        <div>
          Пожалуйста, попробуйте еще раз и нажмите <i>разрешить</i>, чтобы разрешить Flent доступ к вашим локально сохраненным заметкам.
        </div>
      );
      break;
    }
