  ...
  application: {
    ...
    operations: [
      ...
      },
      {
        ...
        title: 'Новая заметка',
        keywords: ['create', 'создать', 'добавить'],
        keybinding: 'Mod-y',
      ...
      },
      {
        ...
        title: 'Скачать резервную копию проекта',
      },
      {
        name: CORE_OPERATIONS_NEW_WORKSPACE_FROM_BACKUP,
        title: 'Восстановить проект из файла резервной копии',
      },
      {
...
async function createNativeFsWorkspace(rootDirHandle: any) {
  ...
  if (typeof wsName === 'string') {
    ...
    throw new Error(
      'Неправильные параметры для ' + CORE_OPERATIONS_CREATE_NATIVE_FS_WORKSPACE,
    );
  }
...
async function createBrowserWorkspace(wsName: WsName) {
  if (typeof wsName !== 'string') {
    throw new Error(
      'Неправильные параметры для ' + CORE_OPERATIONS_CREATE_BROWSER_WORKSPACE,
    );
  }
...
async function createPrivateFsWorkspace(wsName: WsName) {
  if (typeof wsName !== 'string') {
    throw new Error(
      'Неправильные параметры для ' + CORE_OPERATIONS_CREATE_PRIVATE_FS_WORKSPACE,
    );
  }
